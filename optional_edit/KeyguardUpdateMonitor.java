    ...
    protected final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_TIME_TICK.equals(action)
                ...
                // Adding actual current/voltage/wattage
                BatteryManager mBatteryManager = (BatteryManager) context.getSystemService(Context.BATTERY_SERVICE);
                int actChargingMicroAmp = -1 * mBatteryManager.getIntProperty(BatteryManager.BATTERY_PROPERTY_CURRENT_NOW);
                int actChargingMicroVolt = intent.getIntExtra(BatteryManager.EXTRA_VOLTAGE, -1) * 1000;
                int actChargingMicroWatt;
                if (actChargingMicroVolt <= 0) {
                    actChargingMicroVolt = DEFAULT_CHARGING_VOLTAGE_MICRO_VOLT;
                }
                if (actChargingMicroAmp > 0) {
                    actChargingMicroWatt = (actChargingMicroAmp / 1000)
                            * (actChargingMicroVolt / 1000);
                } else {
                    actChargingMicroWatt = -1;
                }
                //End
                ...
                final Message msg = mHandler.obtainMessage(
                        MSG_BATTERY_UPDATE, new BatteryStatus(status, level, plugged, health,
                                actChargingMicroAmp, actChargingMicroVolt, actChargingMicroWatt,
